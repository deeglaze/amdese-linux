...
static ssize_t tsm_report_manifest_selector_write(struct config_item *cfg,
...
CONFIGFS_BIN_ATTR_WO(tsm_report_, manifest_selector, NULL, TSM_MANIFEST_SELECTOR_MAX);
...
static ssize_t default_service_error_show(char *buf, void *service_error)
{
	...
	/* No error implies empty error file. */
	return 0;
}

static ssize_t tsm_report_service_error_show(struct config_item *cfg,
					     char *buf)
{
	struct tsm_report *report = to_tsm_report(cfg);
...
static void tsm_report_item_release(struct config_item *cfg)
{
	...
	kvfree(report->outblob);
	kfree(report->desc.service_provider);
	kvfree(report->desc.manifest_selector);
	kfree(report->service_error);
	kfree(state);
}
